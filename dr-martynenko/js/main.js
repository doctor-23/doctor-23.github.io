...
class FormSender {
  ...
  init(form) {
    ...
    this.props.url = typeof astrooma !== 'undefined' && dr_martynenko.ajaxurl ? dr_martynenko.ajaxurl : 'https://jsonplaceholder.typicode.com/posts';
  ...
  */
          ...
          catalogItems.forEach(item => {
            item.addEventListener('mouseenter', event => {
              const {
                submenuClass,
                category
              } = getParentListAndCategory(event.target);
  ...
  function mainHeroSlider(sliderWrap) {
    ...
    const swiper = new Swiper(sliderWrap, {
      ...
      slidesPerView: 1,
      // Отображать по одному слайду за раз
      loop: false,
