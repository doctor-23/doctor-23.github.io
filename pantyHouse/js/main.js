$(document).ready(function () {
    ...
    var mySelect = $('.customSelect').customSelect();
    ...
    $(document).on('click', '#sort ~ .custom-select-panel .custom-select-option', function () {
        var getUrl = $(this).data('value'),
            url = window.location.origin + window.location.pathname + '?orderby=' + getUrl;

        $(document).find('.catalog-content').addClass('process-page');

        $.ajax({
            url: url,
            type: 'post',
            success: function (dataSuccessHtml) {
                // console.log(dataSuccessHtml)
                var catalog = $(dataSuccessHtml).find('.view__card');

                $(document).find('.view__card ').replaceWith(catalog);

                history.replaceState(null, null, url);
                $(document).find('.catalog-content').removeClass('process-page');
            },
